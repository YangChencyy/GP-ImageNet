...
##################################  Train   ###########################################################
    ...
    ckpt_dir = os.path.join('ckpt', 'mnist')
    ...
    if train:
        ...
        combined_array = combined_array[:10000, :]
        ...
        file_path = os.path.join(ckpt_dir, 'train.csv')
        np.savetxt(file_path, combined_array, delimiter=",", fmt='%f', header=header_string, comments='')
    else:
        print('######################################')
        print('Load model:')
...
##################################  Test   ############################################################
    ...
    np.savetxt(save_path, combined_test_array, delimiter=",", fmt='%f')
...
## SVHN
    ...
    svhn_loader = SVHNDataLoader(root_dir='./data/svhn', batch_size=32, download=True).get_data_loader()
    model.eval()
    ood_features = []  # List to store features
    ood_logits = []  # List to store logits (for softmax scores)
    ood_labels = []  # List to store labels
    ...
    plt.savefig(os.path.join(ckpt_dir, "SVHN.png"))
    ...
    with open(os.path.join(ckpt_dir, "SVHN_test.csv"), "w") as file:
        file.write(",".join(column_names) + "\n")
